...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = distance(texture2D(bgl_DepthTexture, gl_TexCoord[0].xy).xyz,camPos)/zFar;
		d += tmp * kernel[i];
	}
...
void main() 
{
	...
	float depth = distance(texture2D(bgl_DepthTexture, gl_TexCoord[0].xy).xyz,camPos)/zFar;
	float blur = 0.0;
	...
	blur = clamp((abs(depth - focalDepth/2)/range)*100.0,-maxblur,maxblur);
	
	if (autofocus)
	{
		float fDepth = distance(texture2D(bgl_DepthTexture, focus).xyz,camPos)/zFar;
